// Astra syntax highlighting for the guide. The word lists are the lexer's own
// (orbs/astra_lexer/lib.or): if a word is not here it is not reserved. Pure
...
(function () {
  ...
  // Every word keyword_of() maps, in the lexer's own order.
  const KEYWORDS = new Set([
    'rule', 'require', 'let', 'set', 'destroy', 'emit', 'effect', 'on', 'view',
    'entity', 'test', 'apply', 'expect', 'if', 'then', 'else', 'match', 'for',
    'count', 'all', 'in', 'where', 'not', 'and', 'or', 'when', 'every', 'fn',
    'const', 'fact', 'conf', 'law', 'relate', 'after', 'belief', 'confidence',
    'goal', 'constraint', 'intent', 'each', 'to', 'from', 'through', 'use',
    'exists', 'becomes', 'spawned', 'destroyed', 'component', 'state',
    'command', 'needs', 'signal', 'type', 'create', 'source', 'animate',
    'asset', 'trait', 'patch', 'template', 'tokens', 'key', 'uses', 'utility',
    'data'
  ]);
  const TYPES = new Set(['nat', 'int', 'bool', 'text', 'number', 'truth', 'enum']);
  const CONSTS = new Set(['true', 'false', 'none']);
  // Reserved once, never parsed, and taken out so a bundle may use the word.
  const RETIRED = new Set(['spawn', 'record', 'derive']);
